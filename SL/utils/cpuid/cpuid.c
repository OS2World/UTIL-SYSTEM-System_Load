...
void __cpuid(int *pCPUInfo, int iAX)
{
  __asm {
    mov     edi, pCPUInfo
    mov     eax, iAX
...
void __cpuidex(int *pCPUInfo, int iAX, int iCX)
{
  __asm {
    mov     edi, pCPUInfo
    mov     eax, iAX
...
#ifdef DIGI_CODE
  printf( "System Load - CPUID utility." );
#endif
    ...
    *((int*)(CPUString+8)) = CPUInfo[2];
    ...
    for (i=0; i<=nIds; ++i)
    {
        ...
        {
#ifdef DIGI_CODE
            nSteppingID = CPUID_TO_STEPPING( CPUInfo[0] );
            nModel = CPUID_TO_MODEL( CPUInfo[0] );
            nFamily = CPUID_TO_FAMILY( CPUInfo[0] );
#else
    ...
    if  (nIds >= 1)
    {
        ...
        if  (nModel)
            printf_s("Model = 0x%X (%d)\n", nModel, nModel);
        if  (nFamily)
            printf_s("Family = 0x%X (%d)\n", nFamily, nFamily);
        if  (nProcessorType)
