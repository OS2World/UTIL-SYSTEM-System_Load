  ...
  switch( msg )
  {
    ...
    case WM_DESTROY:
      if ( WinQueryCapture( HWND_DESKTOP ) == hwnd )
      {
         ...
         WinSendMsg( LONGFROMMR( WinSendMsg( WinQueryWindow( hwnd, QW_PARENT ), WM_SL_QUERY, MPFROMLONG(SLQUERY_HINT), 0 ) ),
    ...
    case WM_PAINT:
      {
        ...
        // Lets data source to paint window.
    ...
    case WM_CONTEXTMENU:
      ...
      {
        HWND		hwndParent =
                          LONGFROMMR( WinQueryWindow( hwnd, QW_PARENT ) );
        POINTL		ptMap;
    ...
    case WM_SL_UPDATE_DATA:
      ...
      WinSendMsg( LONGFROMMR( WinSendMsg( WinQueryWindow( hwnd, QW_PARENT ), WM_SL_QUERY, MPFROMLONG(SLQUERY_HINT), 0 ) ),
                  WM_HINT_UPDATE, MPFROMHWND( hwnd ), 0 );
      return (MRESULT)FALSE;
...
BOOL itemsInit(HAB hab)
{
  ...
  if ( hpsMem == NULLHANDLE )
  {
    debug( "Warning! Memory PS was not created" );
    DevCloseDC( hdcMem );
    hdcMem = NULLHANDLE;
  }
  else
  ...
  debug( "Thread %u runned", tidUpdate );
...
VOID itemsSetDataSrc(HWND hwndList, PDATASOURCE pDataSrc)
{
  ...
  WinSendMsg( LONGFROMMR( WinSendMsg( hwndList, WM_SL_QUERY, MPFROMLONG(SLQUERY_HINT), 0 ) ),
              WM_HINT_UNSET, 0, 0 );
  ...
  if ( pCurrentDataSrc->fInitialized )
  {
    ...
    for( sWndData.ulItem = 0; sWndData.ulItem < pDataSrc->fnGetCount();
         sWndData.ulItem++ )
    {
      hwndItem = WinCreateWindow( hwndList, WC_SL_ITEM, NULL, WS_VISIBLE | WS_SAVEBITS | WS_TABSTOP,
                              0,
                              0,
                              sizeItem.cx,
                              sizeItem.cy,
