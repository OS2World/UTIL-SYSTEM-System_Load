...
#define UPDATE_INTERVAL		100	// msec.
...
// List of strings IDs which will be placed in list apszSortBy[]
static ULONG aulSortByStrId[SORT_FIELDS] = {
...
static ULONG		ulMenuDrv = ((ULONG)(-1));
...
static PDRIVE _getDrive(ULONG ulDrv)
{
  ULONG		ulIdx;
...
VOID APIENTRY ScanThread(ULONG ulData)
{
         ...
         DosWaitEventSem( hevStop, SCAN_INTERVAL ) == ERROR_TIMEOUT )
  {
    ...
    for( ulScanDrv = 0; ulScanDrv < 26; ulScanDrv++ )
    {
      ...
      if ( !fShowFloppy && ( stInfo.ulType == DI_TYPE_FLOPPY ) )
        continue;
      ...
      if ( pDrive != NULL )
      {
        // Drive for ulScanDrv found in list.

        // "Real" size of field stDI.
        cbDriveInfo = sizeof(DISKINFO) - CCHMAXPATH + pDrive->stDI.cbFSDName + 1;

        if ( cbScanInfo == cbDriveInfo )
        {
          // Sizes of obtained current dirive information in stInfo and
          ...
          // Store in drive's record current iteration of the scan.
      ...
      {
        // Drive not listed - get new position for the new record.
    ...
    for( lDrvIdx = cDrives - 1; lDrvIdx >= 0; lDrvIdx-- )
    {
      ...
      // Record hasn't current scan iteration stamp - destroy and remove it.

      // Choose other selected drive if drive that must be removed selected.
      ...
      // Rmove from the list.
      cDrives--;
      memmove( &apDrives[lDrvIdx], &apDrives[lDrvIdx + 1],
...
DSEXPORT PDSINFO APIENTRY dsInstall(HMODULE hMod, PSLINFO pSLInfo)
{
  ...
  // Load names of fields to sort.
  for( ulIdx = 0; ulIdx < SORT_FIELDS; ulIdx++ )
  {
    pstrLoad( hMod, aulSortByStrId[ulIdx], sizeof(szBuf), &szBuf );
    apszSortBy[ulIdx] = debugStrDup( &szBuf );
  }
...
DSEXPORT VOID APIENTRY dsUninstall()
{
  ...
  for( ulIdx = 0; ulIdx < SORT_FIELDS; ulIdx++ )
  {
    if ( apszSortBy[ulIdx] != NULL )
      debugFree( apszSortBy[ulIdx] );
  }
...
DSEXPORT ULONG APIENTRY dsUpdate(ULONG ulTime)
{
  ...
  ulRC = DosRequestMutexSem( hmtxScan, SEM_IMMEDIATE_RETURN );
  if ( ulRC != NO_ERROR )
  {
...
DSEXPORT VOID APIENTRY dsPaintItem(ULONG ulIndex, HPS hps, PSIZEL psizeWnd)
{
  ...
  if ( pDrive->stDI.ullTotal != 0 )
  {
    ...
    apszVal[1] = &szTotal;
    pt.y -= ulLineCY;
    putilWriteResStr( hps, &pt, hDSModule, IDS_BAR_CAPTION,
                                 2, &apszVal );
  }
...
DSEXPORT VOID APIENTRY dsPaintDetails(HPS hps, PSIZEL psizeWnd)
{
  ...
  {
    putilWriteResStr( hps, &pt, hDSModule, aulStrResId[ulIdx], 0,
                                 NULL );
    pt.y -= ulLineHeight;
  ...
  // Line 1 - drive letter
  *((PUSHORT)&szBuf1) = (CHAR)('A' + pDrive->ulDrv) + ((USHORT)':' << 8);
  GpiCharStringAt( hps, &ptVal, 2, &szBuf1 );
  ...
  if ( pDrive->stDI.ullTotal != 0 )
  {
    _ullToStr( &szBuf1, pDrive->stDI.ullTotal );
    _ullToStr( &szBuf2, pDrive->stDI.ullFree );
...
DSEXPORT VOID APIENTRY dsFillMenu(HWND hwndMenu, ULONG ulIndex)
{
  ...
  if ( ulIndex >= cDrives || !apDrives[ulIndex]->stDI.fPresent )
  {
    ulMenuDrv = (ULONG)(-1);
    DosReleaseMutexSem( hmtxScan );
    return;
  ...
  fRemovable = pDrive->stDI.fRemovable;
