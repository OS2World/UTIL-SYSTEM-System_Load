...
static BOOL _tifOnline(PIPTIF pTIF, BOOL fOnline)
{
  ...
  if ( fOnline )
  {
    ...
    {
      debug( "ioctl(,SIOCGIFEFLAGS,) failed" );
      return FALSE;
    }
  ...
  {
    debug( "ioctl(,SIOCSIFEFLAGS,) failed" );
    return FALSE;
  }
  ...
  // cbBuf = IPTIF_PKT_LENGTH( &stTIF )
    ...
    do
    {
      ...
      if ( pFragPkt == NULL )
      {
        ...
        pFragPkt = debugMAlloc( sizeof(FRAGPKT) );
        if ( pFragPkt == NULL )
        {
      ...
      {
        // Last packet of datagram - we know full length of datagram now.
      ...
      if ( stPktInfo.cbPacket != 0 )
      {
        ...
        else
        {
          ...
          {
            debug( "Too many byte ranges of fragments" );
            lnkseqRemove( &pTIF->lsFrags, pFragPkt );
            debugFree( pFragPkt );
          }
          else
...
{
  PIPTIF	pTIF = debugCAlloc( 1, sizeof(IPTIF) );
  ULONG		ulRC;
...
static VOID _iptFree(PIPTIF pTIF)
{
  ...
  lnkseqFree( &pTIF->lsFrags, PFRAGPKT, debugFree );
